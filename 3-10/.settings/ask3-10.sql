...
SELECT s.*, a.AreaName FROM Shop s INNER JOIN Area a ON s.AreaCode = a.AreaCode WHERE a.AreaName = '関東';
...
SELECT s.ShopCode, s.ShopName, a.AreaName FROM Shop s INNER JOIN Area a ON s.AreaCode = a.AreaCode;

-- 8. 「新宿」店の在庫数が10以上の商品の商品コード（GoodsCode）、商品名（GoodsName）、在庫数（quantity）を抽出して下さい。
SELECT g.GoodsCode, g.GoodsName, st.Quantity FROM Stocks st INNER JOIN Goods g ON st.GoodsCode = g.GoodsCode INNER JOIN Shop s ON st.ShopCode = s.ShopCode WHERE s.ShopName = '新宿' AND st.Quantity >= 10;
